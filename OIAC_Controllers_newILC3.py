...
class ILCv3():
    """
    基于图片公式的迭代学习控制器
    实现公式：U_f^{(i)}(t_i) = (τ_f^{(i)}(t_i) + a * e^{(i)}(t_i)) / s_a(a_f)
    其中：
      U_f^{(i)}: 第i次迭代的前馈扭矩
      τ_f^{(i)}: 第i次迭代的反馈扭矩
      e^{(i)}: 第i次迭代的跟踪误差
      a: 学习增益
      s_a(a_f): 饱和函数，限制前馈扭矩大小
    """
    def __init__(self, max_trials=10, learning_gain=0.1, saturation_limit=20.0):
        """
        初始化ILCv3控制器
        ...
        # 存储学习数据
        self.learned_feedforward = []  # 每个trial的前馈扭矩序列
        self.feedback_torque_history = []  # 每个trial的反馈扭矩序列
        self.error_history = []  # 每个trial的误差序列
        self.reference_time = None  # 参考时间轴
        
        # 性能记录
        self.trial_errors = []
        self.trial_torques = []
        
    def update_learning(self, time_array, error_array, feedback_torque_array):
        """
        更新迭代学习控制器的学习过程
        ...
        参数:
            time_array: 时间序列
            error_array: 跟踪误差序列
            feedback_torque_array: 反馈扭矩序列
        
        返回:
            ff: 更新后的前馈扭矩序列
        """
        if len(time_array) == 0 or len(error_array) == 0 or len(feedback_torque_array) == 0:
            print("[ILCv3] Warning: Empty data, skipping update")
            return np.zeros(len(self.learned_feedforward[0]) if self.learned_feedforward else 100)
        
        # 创建统一的参考时间轴
        if self.reference_time is None:
            max_time = max(time_array) if len(time_array) > 0 else 10.0
            sample_rate = len(time_array) / max_time if max_time > 0 else 50
            self.reference_time = np.linspace(0, max_time, int(sample_rate * max_time))
        ...
        try:
            ...
            # 对齐反馈扭矩数据
            interp_torque = interpolate.interp1d(
                time_array, feedback_torque_array,
                kind='linear',
                bounds_error=False,
                fill_value=0.0
            )
            aligned_feedback_torque = interp_torque(self.reference_time)
        except Exception as e:
            print(f"[ILCv3] Interpolation error: {e}")
            aligned_error = np.zeros_like(self.reference_time)
            aligned_feedback_torque = np.zeros_like(self.reference_time)
        
        # 保存当前trial的数据
        self.error_history.append(aligned_error)
        self.feedback_torque_history.append(aligned_feedback_torque)
        
        # 应用迭代学习公式: U_f^{(i+1)} = (τ_f^{(i)} + a * e^{(i)}) / s_a(a_f)
        # 这里我们简化为直接计算，饱和函数s_a用clip函数实现
        if not self.learned_feedforward:
            # 第一次迭代：使用初始值（零）
            ff = np.zeros_like(aligned_error)
        else:
            # 后续迭代：根据公式计算
            # 注意：公式中用的是第i次迭代的反馈扭矩和误差来计算第i+1次的前馈
            last_idx = len(self.learned_feedforward) - 1
            last_feedback = self.feedback_torque_history[last_idx]
            last_error = self.error_history[last_idx]
            
            # 应用公式：U_f^{(i+1)} = τ_f^{(i)} + a * e^{(i)}
            ff = last_feedback + self.learning_gain * last_error
            
            # 应用饱和函数s_a(a_f) - 用clip实现
            ff = np.clip(ff, -self.saturation_limit, self.saturation_limit)
        ...
        print(f"[ILCv3] Trial {self.current_trial} completed:")
        print(f"      Learning gain: {self.learning_gain}")
        print(f"      Avg error: {math.degrees(avg_error):.2f}°")
        print(f"      Max error: {math.degrees(max_error):.2f}°")
        print(f"      Feedforward range: [{np.min(ff):.2f}, {np.max(ff):.2f}] Nm")
    ...
    def reset(self):
        """重置ILC"""
        self.learned_feedforward.clear()
        self.feedback_torque_history.clear()
        self.error_history.clear()
        self.current_trial = 0
        self.reference_time = None
        print("[ILCv3] Reset completed")
