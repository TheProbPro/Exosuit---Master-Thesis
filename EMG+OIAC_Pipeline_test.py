...
if __name__ == "__main__":
    ...
    desired_pos_smoothing_queue = queue.Queue(maxsize=10)
    plot_position = []
    ...
    #while not stop_event.is_set():
    while last_time - loop_timer < 5:  # Run for 5 seconds
        ...
        print(f"inserted: {desired_angle_deg}")
        numpyarray = np.array(list(desired_pos_smoothing_queue.queue))
        print(f"desired position smoothing queue: {numpyarray}")
        ...
        current_angle_deg = (2550 - motor_state['position']) / step
        ...
        try:
            command_queue.put_nowait((tau_clipped, position_motor))
        except queue.Full:
            try:
