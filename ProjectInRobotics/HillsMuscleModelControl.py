...
from Controllers.MyoelectricControllers import HillMuscleTorqueController
...
if __name__ == "__main__":
    ...
    print("Press enter to run Hills muscle model control for 10 seconds...")
    input()
    last_time = time.time()
    ...
    try:
        while not stop_event.is_set():
                ...
                # TODO: Add tbe muscles here
                control_output = controller.step()
                control_output = np.clip(control_output, TORQUE_MIN, TORQUE_MAX)
                plot_torque.append(control_output)
