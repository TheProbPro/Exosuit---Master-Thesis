...
class TrueRANMultifunctionalController:
    """
    True RAN multifunctional controller - 从仿真移植到硬件
    实现AAN/RAN模式自动切换
    RAN模式：在跟踪良好时提供阻力，不使用ILC前馈
    """
    def __init__(self, oiac, ilc):
        ...
        # RAN activation parameters (from simulation)
        # 硬件建议：可以根据实际表现调整这些阈值
        self.error_threshold_aan_to_ran = math.radians(3.0)  # AAN→RAN: 误差小于3°
        self.error_threshold_ran_to_aan = math.radians(7.0)  # RAN→AAN: 误差大于7°
        
        # RAN resistance parameters (from simulation)
        # 硬件建议：从较小的阻力开始，逐步调整
        ...
        self.min_switch_interval = 0.5  # Minimum time between switches (防止抖动)
